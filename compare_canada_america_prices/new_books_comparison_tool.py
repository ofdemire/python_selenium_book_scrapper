	...
	with open( kal +'.csv', 'w', newline = '') as f:
		...
		for index in books:
			try:
				...
				my_list = nameList[0].findAll("img")
				priceList = nameList[0].findAll("span" , {"class": "results-price"})
				...
				if(my_list[0]['title'] != "Amazon.com" and my_list[0]['title']!= "Amazon.com (Prime)"):
					for line in my_list:
						if(line['title'] == "Amazon.com" or line['title'] == "Amazon.com (Prime)"):
							price_index = count
							break
							#break i sil eger garanti lerden cikmak istiyosan ama suan garanti kar edicekleri buluyor suzgeci daraltiyor baya
					...
					for line in my_list:
						if "Amazon.ca" in line["title"]:
							if(exclude_data(nameList[0], counter) == 0):
								counter=counter+1
								continue
